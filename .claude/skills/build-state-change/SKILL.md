...
The URL names the **business intention** in the domain's own words, not the slice. Every command path has the shape `POST /{entities}[/{entity_id}]/{action}` — entity collection, the id when the entity already exists, and an active verb last. Work it out before writing the module; the full rule and its table live in `.build-kit/CLAUDE.md` → *API addressing*. In short:
...
2. **Keep the command's verb, drop the noun the path already carries, add the object if there is one.** `AdminCancelLicence` under `/licences/{licence_id}/` → `cancel`; `ChangeCourseCapacity` under `/courses/{course_id}/` → `change-capacity`; `SubscribeStudent` under `/students/{student_id}/` → `subscribe-to-course`. Imperative verb — never a gerund (`cancelling`) and never a noun-phrase resource (`cancellation-requests`). Keep a qualifier that disambiguates: `admin-cancel` when a `CustomerCancelLicence` also exists.
3. **Commands that create their own entity** have no id to nest under, but still sit under the collection: `POST /users/register`. The id stays in the **body** even when the client supplies it — an id for a thing that does not exist yet is not an address. Same shape for a genuinely global boundary (`tags=[]`), using the command's subject as the collection.

```
AdminCancelLicence + tags=[f"licence:{licence_id}"]
    -> POST /licences/{licence_id}/cancel

RegisterUser (creates the entity)
...
router = APIRouter(tags=["licences"])  # the entity, pluralised — see step 5 above
...
# The router tag above, and the path, the id parameter and the id kwarg below,
# are the *worked example* (`AdminCancelLicense` over
# `tags=[f"licence:{licence_id}"]`), not placeholder tokens — substitute the
# entity and action you settled on above. Everything else in this template is
# verbatim.
@router.post(
...
- **`status_code` is `HTTP_201_CREATED`, never `HTTP_200_OK`.** A command that succeeds appends events to the log, which is a creation whatever the slice's verb suggests — `AdminCancelLicense` creates a `LicenceCancelled` event. A command route therefore answers 201 on success and 204 on a no-op.
