...
def test_course_catalogue_empty_returns_empty_list(
    client: TestClient,
    manager_auth: dict,
) -> None:
    """With no courses registered, the catalogue is an empty list, not a 404."""
    response = client.get("/course-catalogue", headers=manager_auth)
...
def test_course_catalogue_reports_too_early_when_behind(
    ...
    response = client.get(
        "/course-catalogue",
        headers={**manager_auth, "X-Position-AtLeast": "1000000"},
    )
    assert response.status_code == 425
    assert response.content == b""


def test_course_catalogue_is_also_visible_to_a_student(
    client: TestClient,
    student_auth: Callable[[str], dict],
    first_course_registered: CourseRegistered,  # noqa: ARG001
) -> None:
    """
    A student sees the same catalogue a course manager does.

    The board draws this screen in both lanes, so this is the one route where
    the rule is identity alone. Asserting it keeps a later tightening of the
    catalogue from silently cutting students off from the course list they
    subscribe from.
    """
    response = client.get("/course-catalogue", headers=student_auth(_STUDENT_ID))
    assert response.status_code == 200
    assert response.json()["courses"][0]["course_id"] == _FIRST_COURSE_ID


def test_course_catalogue_without_a_token_returns_401(client: TestClient) -> None:
    """An anonymous caller does not see the catalogue."""
